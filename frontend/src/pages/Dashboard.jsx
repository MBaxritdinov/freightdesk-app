      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-400 mt-1">
            Week of {new Date(stats.week_start + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
            {' – '}
            {new Date(stats.week_end + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard
            ...
            sub={stats.pending_count === 1 ? 'load needs review' : 'loads need review'}
          />
        </div>

        {/* Empty state — no loads this week */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
          ...
          <div className="lg:col-span-2 bg-slate-800 rounded-xl border border-slate-700 p-5">
            ...
            {stats.total_loads_week === 0 ? (
              <div className="flex items-center justify-center h-40 text-slate-500 text-sm">No loads this week</div>
            ) : (
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
            ...
            {stats.total_loads_week === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie
                    data={stats.loads_by_status.filter(s => s.count > 0)}
                    dataKey="count"
                    nameKey="status"
                    cx="50%"
                    cy="50%"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  {['Load #', 'Broker', 'Driver', 'Route', 'Gross', 'Status', 'Created'].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wide whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {stats.recent_loads.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-10 text-slate-500">No loads yet</td>
                  </tr>
                ) : (
                  stats.recent_loads.map(l => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-300 whitespace-nowrap">
                        {l.driver_name ?? <span className="text-slate-600">—</span>}
                      </td>
                      ...
                      <td className="px-4 py-3 text-white font-mono whitespace-nowrap">
                        {'$' + Number(l.gross_rate).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </td>
