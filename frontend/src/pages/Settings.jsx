    ...
    <div className="bg-slate-800 rounded-xl border border-slate-700 p-6 mt-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-sm">
        <div>
          ...
          <input name="current_password" type="password" value={form.current_password} onChange={set} required className={inp} />
        </div>
        <div>
          <label className={lbl}>New Password</label>
          <input name="new_password" type="password" value={form.new_password} onChange={set} required minLength={6} className={inp} placeholder="Min. 6 characters" />
        </div>
        <div>
          <label className={lbl}>Confirm New Password</label>
          <input name="confirm_password" type="password" value={form.confirm_password} onChange={set} required className={inp} />
        </div>
        {result && (
          <p className={`text-sm ${result.error ? 'text-red-400' : 'text-green-400'}`}>
            {result.error ?? '✓ Password changed successfully'}
          </p>
        )}
        <button
          type="submit"
          ...
          <div className="flex items-start justify-between gap-4 mb-1">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">
                Connect a Gmail inbox to automatically parse broker rate confirmation emails and
                create loads. The inbox is polled every 15 minutes.
              </p>
            </div>
            {gmailStatus && (
              connected
                ? (
                  <span className="shrink-0 px-2.5 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400 border border-green-500/30">
                    Connected
                  </span>
                )
                : (
                  <span className="shrink-0 px-2.5 py-1 rounded-full text-xs font-medium bg-red-500/20 text-red-400 border border-red-500/30">
                    Not connected
                  </span>
                )
            )}
          ...
          <div className="mt-5 space-y-4">
            {gmailStatus === null ? (
              ...
              <>
                ...
                {/* Action buttons — HEAD_ACCOUNTANT only */}
                {isHA && (
                  <div className="flex items-center gap-3 pt-1">
                    {!connected && (
                      <button
                        ...
                        disabled={connecting}
                        className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white text-sm rounded-lg font-medium transition flex items-center gap-2"
                      >
                        {connecting && <Spinner />}
                    ...
                    {connected && (
                      <button
                        onClick={handleSync}
                        disabled={syncing}
                        className="px-4 py-2 bg-slate-700 hover:bg-slate-600 disabled:opacity-60 text-white text-sm rounded-lg font-medium border border-slate-600 transition flex items-center gap-2"
                      >
                        {syncing && <Spinner />}
                    ...
                    {connected && (
                      <button
                        onClick={handleConnect}
                        disabled={connecting}
                        className="px-4 py-2 text-sm text-slate-400 hover:text-white border border-slate-600 hover:border-slate-500 rounded-lg transition flex items-center gap-2"
                      >
                        {connecting && <Spinner />}
                ...
                {connecting && (
                  <div className="flex items-start gap-2 px-3 py-2.5 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                    ...
                    <p className="text-xs text-blue-300">
                      A browser window has opened for Google authorization. Sign in and grant
                      access to continue. This page will update automatically when done.
                    </p>
                  </div>
                )}
                ...
                {syncResult && (
                  <div className={`px-3 py-2.5 rounded-lg border text-sm ${
                    ...
                    {syncResult.error ? (
                      syncResult.error
                    ) : syncResult.skipped && syncResult.reason ? (
                      <span className="text-slate-400">{syncResult.reason}</span>
                    ) : (
                      <>
                        Sync complete —{' '}
                        <span className="text-green-400">{syncResult.created ?? 0} load{syncResult.created !== 1 ? 's' : ''} created</span>
                        {', '}
                        <span className="text-slate-400">{syncResult.skipped ?? 0} skipped</span>
                        {syncResult.errors > 0 && (
                          <>, <span className="text-red-400">{syncResult.errors} errors</span></>
                        )}
                      </>
                    )}
                  </div>
                ...
                {!isHA && !connected && (
                  <p className="text-sm text-slate-500">
                    Ask your Head Accountant to connect a Gmail inbox.
                  </p>
                )}
              </>
            )}
        ...
        <div className="bg-slate-800 rounded-xl border border-slate-700 p-6 mt-4">
          <div className="flex items-start justify-between gap-4 mb-1">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">
                Send weekly settlement summaries to drivers via Telegram.
              </p>
            </div>
            {tgStatus && (
              tgStatus.configured
                ...
                : (
                  <span className="shrink-0 px-2.5 py-1 rounded-full text-xs font-medium bg-slate-600/40 text-slate-400 border border-slate-500/30">
                    Not configured
                  </span>
                )
            )}
            ...
            {tgStatus === null ? (
              ...
              <div className="space-y-3">
                <p className="text-sm text-slate-400">
                  Set <code className="text-blue-400 text-xs bg-slate-700 px-1.5 py-0.5 rounded">TELEGRAM_BOT_TOKEN</code> in{' '}
                  <code className="text-blue-400 text-xs bg-slate-700 px-1.5 py-0.5 rounded">backend/.env</code> and restart the server.
                </p>
                <div className="px-3 py-2.5 bg-slate-700/50 rounded-lg border border-slate-600 text-xs text-slate-400 space-y-1">
                  ...
                  <p>4. Add it to <code className="text-blue-400">backend/.env</code> as <code className="text-blue-400">TELEGRAM_BOT_TOKEN=...</code></p>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="px-3 py-2.5 bg-slate-700/40 rounded-lg border border-slate-600/50 text-sm text-slate-400">
                  Drivers get their Chat ID by messaging{' '}
                  <span className="text-white">@{tgStatus.bot_username}</span> on Telegram.
                  Any message they send will show their ID in the bot's update list.
                  You can also use{' '}
                  <span className="text-blue-400">@userinfobot</span> — forward a message from the
                  driver to it and it returns their Chat ID.
                </div>
                ...
                {isHA && (
                  <div className="space-y-2">
                    ...
                    {testResult && (
                      <p className={`text-sm ${testResult.error ? 'text-red-400' : 'text-green-400'}`}>
